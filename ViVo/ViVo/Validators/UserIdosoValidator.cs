...
{
    class UserIdosoValidator : AbstractValidator<UserIdoso>
    {

        public UserIdosoValidator()
        {
            ...
            RuleFor(x => x.CpfTutor).Must(Utils.CpfValidator).WithMessage("CPF do Tutor inválido.");
            RuleFor(x => x.Sangue).NotEmpty().WithMessage("O tipo sanguíneo é obrigatório.");
        }
    }
}
